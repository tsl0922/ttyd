...
test('normalizeCustomMobileKeys validates ids and combo', () => {
    ...
    assert.equal(
        normalizeMobileKeyboardCustomKeys([{ id: 'toString', label: 'bad', combo: ['Ctrl+b'] }]).valid,
        false,
        'toString id should be rejected'
    );
    assert.equal(
        normalizeMobileKeyboardCustomKeys([{ id: 'valueOf', label: 'bad', combo: ['Ctrl+b'] }]).valid,
        false,
        'valueOf id should be rejected'
    );
    assert.equal(
...
test('custom key id validator enforces strict naming pattern', () => {
    ...
    assert.equal(isValidMobileKeyboardCustomKeyId('toString'), false);
    assert.equal(isValidMobileKeyboardCustomKeyId('valueOf'), false);
});
